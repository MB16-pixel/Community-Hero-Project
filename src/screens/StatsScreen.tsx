...
export const StatsScreen: React.FC = () => {
  const { user } = useAuth();
  const [leaderboard, setLeaderboard] = useState<CommunityLeaderboardItem[]>([]);
  const [totalCitizens, setTotalCitizens] = useState(0);
  const [loading, setLoading] = useState(true);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);

  // Sync users collection to calculate live community leaderboard scores
  useEffect(() => {
    setLoading(true);
    setErrorMsg(null);
    const path = 'users';
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        const sortedList: CommunityLeaderboardItem[] = Object.keys(communityMap).map(key => ({
          community: key,
        ...
        setTotalCitizens(citizensCount);
        setLoading(false);
      },
      (err) => {
        const wrappedError = handleFirestoreError(err, OperationType.LIST, path, user?.uid, user?.email);
        setErrorMsg("Failed to synchronize leaderboard from database.");
        setLoading(false);
